...
- **Dynamic ATR Trailing Stop & Breakeven**: Integrated a dynamic ATR-based risk management system ($0.60 \times \text{ATR}$ breakeven locks and $0.80 \times \text{ATR}$ trigger with $0.35 \times \text{ATR}$ trailing stops) in the backtester and daemon, increasing the M15 intraday backtest win rate from **47.8%** to **77.4%** and profit factor to **2.27**.
