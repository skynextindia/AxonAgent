...
def mt5_initialize(
    ...
    if terminal_path is None:
        terminal_path = os.environ.get("AXONAI_MT5_TERMINAL_PATH")
    if login is None:
        raw_login = os.environ.get("AXONAI_MT5_LOGIN")
        if raw_login:
            try:
                login = int(raw_login)
            except ValueError:
                pass
    if password is None:
        password = os.environ.get("AXONAI_MT5_PASSWORD")
    if server is None:
        server = os.environ.get("AXONAI_MT5_SERVER")
