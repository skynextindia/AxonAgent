...
class BacktestEngine:
    ...
    def _update_simulated_positions(self, bid: float, ask: float, timestamp: datetime):
        ...
        for trade in list(self.active_trades):
            ...
            atr = trade.get("atr", 0.0012)
            
            if dir == "BUY":
                # Update peak price using the bid price
                trade["peak_price"] = max(trade.get("peak_price", entry), bid)
                ...
                trade["peak_price"] = min(trade.get("peak_price", entry), ask)
                peak_price = trade["peak_price"]
                ...
                if profit >= be_trigger:
                    breakeven_sl = entry - 1 * self.pip_mult
                    if trade["sl"] > breakeven_sl or trade["sl"] == 0.0:
                        trade["sl"] = breakeven_sl
                        sl = breakeven_sl
                
                # Check Trailing Stop (once peak profit >= 0.80 * ATR, trail by 0.35 * ATR)
                peak_profit = entry - peak_price
                trail_trigger = 0.80 * atr
                if peak_profit >= trail_trigger:
                    trail_distance = 0.35 * atr
                    trail_sl = peak_price + trail_distance
                    if trade["sl"] > trail_sl or trade["sl"] == 0.0:
                        trade["sl"] = trail_sl
                        sl = trail_sl
