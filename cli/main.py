...
from axonai.graph.analyst_execution import (
...
app = typer.Typer(
    name="AxonAI",
    help="AxonAI CLI: Multi-Agents LLM Financial Trading Framework",
...
def update_display(layout, spinner_text=None, stats_handler=None, start_time=None):
    ...
    layout["header"].update(
        Panel(
            "[bold green]Welcome to AxonAI CLI[/bold green]\n"
            "[dim]© [AxonAI](https://github.com/AxonAI)[/dim]",
            title="Welcome to AxonAI",
            border_style="green",
            padding=(1, 2),
            expand=True,
        )
...
def get_user_selections(
    ...
    if not non_interactive:
        ...
        welcome_content += "[bold green]AxonAI: Multi-Agents LLM Financial Trading Framework - CLI[/bold green]\n\n"
        welcome_content += "[bold]Workflow Steps:[/bold]\n"
        welcome_content += "I. Analyst Team → II. Research Team → III. Trader → IV. Risk Management → V. Portfolio Management\n\n"
        welcome_content += (
            "[dim]Built by [AxonAI](https://github.com/AxonAI)[/dim]"
        )
    ...
    if ticker:
        ...
        selected_ticker = normalize_ticker_symbol(os.environ.get("AXONAI_TICKER") or DEFAULT_CONFIG.get("benchmark_ticker") or "SPY")
    else:
        console.print(
            create_question_box(
                "Step 1: Ticker Symbol",
    ...
    if date:
        ...
        analysis_date = os.environ.get("AXONAI_TRADE_DATE") or datetime.datetime.now().strftime("%Y-%m-%d")
    else:
    ...
    if language:
        ...
        output_language = os.environ.get("AXONAI_OUTPUT_LANGUAGE") or DEFAULT_CONFIG.get("output_language", "English")
    else:
        console.print(
            create_question_box(
                "Step 3: Output Language",
    ...
    if analysts:
        ...
        for a in analysts.split(","):
            val = a.strip().lower()
            if val == "social" or val == "sentiment":
                parsed_analysts.append(AnalystType.SOCIAL)
            elif val in [e.value for e in AnalystType]:
                parsed_analysts.append(AnalystType(val))
    ...
    if depth is not None:
        ...
        selected_research_depth = int(os.environ.get("AXONAI_MAX_DEBATE_ROUNDS") or DEFAULT_CONFIG.get("max_debate_rounds", 1))
    else:
        console.print(
            create_question_box(
...
def run_analysis(
    ...
    # Build World State before any LLM call
    from axonai.world_state import build_world_state
    from dataclasses import asdict

    ticker_sym = selections["ticker"]
    world_state_obj = build_world_state(ticker_sym)

    if not world_state_obj.should_run_graph:
        console.print(f"[bold red]ABORTED: belief={world_state_obj.belief_score:.2f} | regime={world_state_obj.dominant_regime}({world_state_obj.regime_confidence:.2f}) | reason={world_state_obj.abort_reason} | spread={world_state_obj.spread_pips:.1f}pips[/bold red]")
        raise typer.Exit(0)
    else:
        console.print(f"[bold green]WorldState: regime={world_state_obj.dominant_regime}({world_state_obj.regime_confidence:.2f}) | session={world_state_obj.session} | belief={world_state_obj.belief_score:.2f} | EUR={world_state_obj.eur_strength:+.2f} | USD={world_state_obj.usd_strength:+.2f} → RUNNING GRAPH[/bold green]")
    ...
    with Live(layout, refresh_per_second=4) as live:
        ...
        from axonai.dataflows.evidence_extractor import extract_market_evidence
        market_evidence_obj = extract_market_evidence(selections["ticker"])

        init_agent_state = graph.propagator.create_initial_state(
            selections["ticker"],
            selections["analysis_date"],
            asset_type=selections["asset_type"],
            world_state=asdict(world_state_obj),
            market_evidence=asdict(market_evidence_obj),
        )
    ...
    if selections.get("non_interactive"):
...
def analyze(
    ...
    ticker: Optional[str] = typer.Option(
        ...
        "--ticker",
        "-t",
        help="Ticker symbol to analyze (e.g. SPY, EURUSD=X).",
    ),
    ...
    analysts: Optional[str] = typer.Option(
        ...
        "--analysts",
        "-a",
        help="Comma-separated list of analyst types.",
    ),
    depth: Optional[int] = typer.Option(
        None,
        "--depth",
        help="Research depth level (number of debate rounds).",
    ),
    ...
    shallow_thinker: Optional[str] = typer.Option(
        None,
        "--shallow-thinker",
        help="Shallow thinking model name.",
    ),
    ...
    if clear_checkpoints:
        from axonai.graph.checkpointer import clear_all_checkpoints
...
def live(
    ...
    shallow_thinker: Optional[str] = typer.Option(None),
    deep_thinker: Optional[str] = typer.Option(None),
    cooldown: int = typer.Option(300, "--cooldown", help="Seconds between graph fires"),
