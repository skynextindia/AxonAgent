...
from axonai.llm_clients.api_key_env import get_api_key_env
from axonai.llm_clients.model_catalog import get_model_options
...
def _select_model(provider: str, mode: str, default_model: Optional[str] = None) -> str:
    ...
    # If default_model is not custom/empty and not in standard choices, inject it dynamically
    if default_model and default_model != "custom" and not any(c.value == default_model for c in choices if c.value != "custom"):
        custom_idx = next((i for i, c in enumerate(choices) if c.value == "custom"), len(choices))
        choices.insert(custom_idx, questionary.Choice(f"{default_model} (configured in env)", value=default_model))

    default_choice = None
    if default_model:
        for c in choices:
            if c.value == default_model:
...
def select_shallow_thinking_agent(provider, default_model: Optional[str] = None) -> str:
    """Select shallow thinking llm engine using an interactive selection."""
    return _select_model(provider, "quick", default_model)


def select_deep_thinking_agent(provider, default_model: Optional[str] = None) -> str:
    """Select deep thinking llm engine using an interactive selection."""
    return _select_model(provider, "deep", default_model)

def select_llm_provider(default_provider: Optional[str] = None) -> tuple[str, str | None]:
    ...
    default_choice = None
    if default_provider:
        default_lower = default_provider.lower()
        for display, provider_key, url in PROVIDERS:
            if provider_key == default_lower:
