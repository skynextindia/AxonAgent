...
| Backtesting integration | ✅ Production | `axonai/realtime/backtester.py` + `run_intraday_backtest.py` + `run_bridge_backtest.py` fully operational |
| SL/TP Management | ✅ Production | ATR-based dynamic SL/TP fully integrated into `trade_executor.py` and `backtester.py` |
| Position sizing | ✅ Production | Account-equity-risk and volatility-adjusted sizing in `trade_executor.py` |
| Dynamic ATR Trailing SL & BE | ✅ Production | 0.60×ATR breakeven and 0.80×ATR / 0.35×ATR trailing stops in `backtester.py` and `daemon.py` |
