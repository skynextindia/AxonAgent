#!/usr/bin/env python3
"""1-Year Intraday backtest runner for XAUUSD (Gold) fetching real data directly from local MT5.
...
logger = logging.getLogger("xauusd_bt")
logging.getLogger("axonai").setLevel(logging.WARNING)

# 1. Fetch XAUUSD M15 data from MetaTrader 5
symbol = "XAUUSD"
...
for c in candle_rows:
    ...
    t = c["time"]
    n_ticks = 15
    half_spread = 0.05  # Gold standard half-spread: 5 cents (0.5 pips at 0.01 multiplier)
    ...
    spread_jitter = rng.uniform(-0.01, 0.01, n_ticks)  # Gold spread jitter
...
logger.info("Starting 1-YEAR backtest on XAUUSD M15 data (%d bars)...", len(candle_rows))
report = engine.run()
...
md_path = out_dir / f"1year_bt_XAUUSD_{ts}.md"
...
print()
print("=" * 65)
print("  1-YEAR INTRADAY BACKTEST RESULTS (XAUUSD)")
print("=" * 65)
